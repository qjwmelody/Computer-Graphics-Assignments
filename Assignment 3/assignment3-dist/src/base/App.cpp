...
void App::renderSkeleton() {
	...
	for (auto i = 0u; i < transforms.size(); ++i) {
		...
		/*Vec4f temp = transforms[i].getRow(0);
		right = Vec3f(temp[0], temp[1], temp[2]);
		temp = transforms[i].getRow(1);
		up = Vec3f(temp[0], temp[1], temp[2]);
		temp = transforms[i].getRow(2);
		ahead = Vec3f(temp[0], temp[1], temp[2]);*/
		...
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
		glVertex3f(joint_world_pos.x + right.x * scale, joint_world_pos.y + right.y * scale, joint_world_pos.z + right.z * scale);

		// ..and the y axis.. ("up")
		glColor3f(0, 1, 0); // green
		// glVertex3f(...); glVertex3f(...);
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
		glVertex3f(joint_world_pos.x + up.x * scale, joint_world_pos.y + up.y * scale, joint_world_pos.z + up.z * scale);

		// ..and the z axis ("ahead").
		glColor3f(0, 0, 1); // blue
		// glVertex3f(...); glVertex3f(...);
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
		glVertex3f(joint_world_pos.x + ahead.x * scale, joint_world_pos.y + ahead.y * scale, joint_world_pos.z + ahead.z * scale);
		...
		int myParent = skel_.getJointParent(i);
		//if (myParent == -1)
		//	return;
		//else {
		if (myParent != -1) {
			...
			// glVertex3f(...); glVertex3f(...);
			glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
			glVertex3f(transforms[myParent](0, 3), transforms[myParent](1, 3), transforms[myParent](2, 3));
		}
...
vector<Vertex> App::computeSSD(const vector<WeightedVertex>& source_vertices) {
	...
	for (const auto& sv : source_vertices) {
		...
		// Initialization
		v.position = v.normal = v.color = Vec3f(0, 0, 0);
		for (int i = 0; i != WEIGHTS_PER_VERTEX; i++) {
			v.position += ssd_transforms[sv.joints[i]] * sv.position * sv.weights[i];
			v.color += sv.color * sv.weights[i];
			v.normal += ssd_transforms[sv.joints[i]].getXYZ() * sv.normal * sv.weights[i];
		}
