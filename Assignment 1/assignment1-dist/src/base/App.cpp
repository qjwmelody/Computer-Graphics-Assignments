...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
		...
		v1.position = Vec3f(radius * FW::cos(i * angle_increment), -height, radius * FW::sin(i * angle_increment));
		v2.position = Vec3f(radius * FW::cos((i + 1) * angle_increment), -height, radius * FW::sin((i + 1) * angle_increment));
		// Normal
		Vec3f a, b, n;
		a = v0.position - v1.position;
		b = v0.position - v2.position;
		n = cross(a, b);
		n.normalize();
...
bool App::handleEvent(const Window::Event& ev) {
	...
	if (ev.type == Window::EventType_KeyDown) {
		...
		// if (ev.key == FW_KEY_UP)
		//		model_translation_ += Vec3f(0, 0.03, 0);
		// else if (ev.key == FW_KEY_DOWN)
		//		model_translation_ -= Vec3f(0, 0.03, 0);
		// Translation along y aixs
		if (ev.key == FW_KEY_UP)
			model_translation_.y += 0.03;
		else if (ev.key == FW_KEY_DOWN)
			model_translation_.y -= 0.03;
		// x axis
		if (ev.key == FW_KEY_LEFT)
			model_translation_.x -= 0.03;
		else if (ev.key == FW_KEY_RIGHT)
			model_translation_.x += 0.03;
		// z axis
		if (ev.key == FW_KEY_PAGE_UP)
			model_translation_.z += 0.03;
...
void App::initRendering() {
	...
	auto shader_program = new GLContext::Program(
		...
		void main()
		{
			...
			// aNormal = normalize(gl_NormalMatrix * aNormal);
...
void App::render() {
	...
	// Mat4f modelToWorld(Mat4f::translate(model_translation_));	
	// Extra: order: first scale, second rotate, third translate
	Mat4f R;
	Mat3f rotat = Mat3f::rotation(Vec3f(0, 1, 0), -model_rotation_);
	R.setCol(0, Vec4f(rotat.getCol(0), 0));
	R.setCol(1, Vec4f(rotat.getCol(1), 0));
	R.setCol(2, Vec4f(rotat.getCol(2), 0));
	R.setCol(3, Vec4f(0, 0, 0, 1));
	Mat4f modelToWorld(Mat4f::translate(model_translation_) * Mat4f::scale(model_sx_) * R);
	...
	common_ctrl_.message(sprintf("Use arrow keys, PgUp and PgDn to move the model, Home/End to rotate camera."), "instructions");
...
vector<Vertex> App::loadObjFileModel(string filename) {
	...
	while(getline(input, line)) {
		...
		if (s == "v") { // vertex position
			...
			float f1, f2, f3; // Temporary objects to read data into.
			iss >> f1 >> f2 >> f3;
			...
			unsigned int i; // Temporary variable for reading indices
			/*for (int j = 1; j <= 9; j++) {
				iss >> i;
				if (j == 2 || j == 5 || j == 8)
					sink = i;
