...
public:
	...
	static inline FW::Vec2f normalizedImageCoordinateFromPixelCoordinate(const FW::Vec2f& pixel, const FW::Vec2i& imageSize) {
		...
		//return FW::Vec2f();
		FW::Vec2f normalizedCoordinate = FW::Vec2f();
		normalizedCoordinate.x = 2 * pixel.x / (float)imageSize.x - 1;
		normalizedCoordinate.y = -2 * pixel.y / (float)imageSize.y + 1;
...
public:
	...
	virtual Ray generateRay(const FW::Vec2f& point) {
		...
		auto origin = (this->center + point.y * (this->size * this->up) / 2 + point.x * (this->size * this->horizontal) / 2);
...
public:
	...
	{	
		...
		//return Ray( FW::Vec3f(0.0f), FW::Vec3f(0.0f) );
		float distance = 1 / FW::tan(this->fov_angle / 2);
		FW::Vec3f directionV = point.x * this->horizontal + point.y * this->up + distance * this->direction;
		directionV.normalize();
		return Ray(this->center, directionV);
